...
std::map<std::string, std::string> RequestBuilder::GetHttpHeader() {
    ...
    retMap["Content-Type"] = "application/json";
...
std::string RequestBuilder::GetAuthorization() {
    ...
    snprintf(pBuffer, MAX_BUFFER_LEN - 1, "GETtts.cloud.tencent.com/stream_ws?%s",
        GetURLParamString(false).c_str());
    ...
    std::cout << "signature origin_str=" << pBuffer << ", encoded_str=" << strRet << std::endl;
